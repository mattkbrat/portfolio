<script lang="ts">
	...
	const setBoard = (init: { size: number; state?: string }) => {
		if (init.state) {
			...
			board = Array.from(new Array(init.size).keys()).map((i) => threeArr(i, init.size)) as Board;
		}
	};

	let nextPlay = $derived((plays % 2 ? players[1].character : players[0].character)[0]);
...
{#if setup || winner}
	...
	<div class="flex flex-wrap">
		<button
			type="button"
			class="btn btn-lg preset-filled-primary-500 flex-1 outline outline-[1px]"
			onclick={() => {
				reset(3);
			}}
		>
		...
		{#each board as row, n}
			<div class="flex">
				{#each row as square, x}
					<button
						class="aspect-square border-2 relative text-4xl border-surface-100-900 w-full text-surface-50-950"
						name="game-square"
						data-player={square.player}
						class:!bg-success-400-600={square.player === players[0].character}
						class:!bg-error-400-600={square.player === players[1].character}
						class:bg-surface-50-950={!square.player}
						class:hover:bg-surface-200-800={!square.player}
						onclick={() => {
							play(square);
						}}
					>
...
<div class="flex gap-x-2 items-center justify-between flex-wrap">
	{#if gameState === 'play'}
		<button
			type="button"
			class="btn btn-md preset-outlined-warning-500"
			onclick={() => {
	...
	{/if}
	{#if !isTie && !winner && gameState === 'play'}
	...
	{#if cacheState?.length}
		<button
			type="button"
			class="btn btn-md preset-outlined-tertiary-500"
			onclick={() => {
				undo();
			}}
		>
...
<button
	type="button"
	class="btn btn-lg w-full lg:hidden preset-tonal-tertiary"
	onclick={() => {
		scrollToBoardTop();
	}}
>
