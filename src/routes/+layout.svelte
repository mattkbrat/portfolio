...
>
	<nav id="main-nav" class="flex flex-1 md:h-[var(--h-header)]">
		<ul class="mx-8 flex w-full !list-none flex-wrap gap-x-4 gap-y-2 py-4 text-lg">
			...
			<li class="hidden text-primary-300 md:block">
				<a
					class:md:opacity-0={$headingIsVisible}
					class="border-l-2 border-l-current px-2 no-underline transition-all duration-300 hover:!underline"
					href="/"
				>
					Matthew Bratrsovsky
				</a>
...
<div class="flex flex-col-reverse items-center justify-center lg:flex-col">
	{#if isOnResume}
		<div
			class="sticky top-[unset] right-[unset] bottom-0 left-0 mx-auto ml-auto btn-group w-full flex-wrap items-end justify-around rounded-bl-md bg-white lg:fixed lg:top-16 lg:right-0 lg:bottom-[unset] lg:left-[unset] lg:mx-0 lg:w-fit lg:flex-col lg:bg-white/75 dark:bg-black lg:dark:bg-white/25 print:hidden"
		>
			<a
				class="flex flex-row-reverse gap-2 lg:flex-row"
				href="Resume.pdf"
	...
	{/if}
	<div
		class="outline-primary-300-900 m-2 bg-white/75 p-4 outline-1 -outline-offset-4 lg:my-8 lg:p-8 dark:bg-black/75 print:!mx-8 print:outline-transparent"
	>
		<div class="break-word flex max-w-[75ch] flex-col self-center xl:max-w-[120ch] print:max-w-max">
			{@render children()}
		</div>
	</div>
</div>
<section id="github-corner-section" class="hidden md:block print:hidden">
	...
	<a id="github-corner" target="_blank" href="https://github.com/mattkbrat/portfolio">
		<div
			class="fixed bottom-0 left-0 -mb-[100px] -ml-[150px] h-[200px] w-[200px] rotate-45 cursor-pointer border-t-2 border-surface-200-800 bg-black/5 text-[0px]"
		>
			Github
		</div>
	</a>
